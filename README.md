...
При оценке QPS будем учитывать, что аудитория сервиса распределена по всему миру и инфраструктура будет продублирована в двух датацентрах, то есть число запросов на 1 ДЦ составит 1/2 от общего числа запросов.
...
Тогда:
* На таблицы **nodes**, **relation_members**, **relations**, **way_nodes**, **ways**: `5 * 7407 * 0.5 / 1000 = 19 QPS`
* На таблицу **tiles**: `7407 * 0.5 = 3204 QPS`

Запросы к **address** выполняются при геокодинге, требуется 1 запрос к таблице на 1 запрос геокодинга. При этом 20% запросов будет кэшироваться, то есть число запросов к таблице составит 0.8%. Тогда: `93 * 0.8 * 0.5 = 38 QPS`

Допустим, что 75% запросов представляют собой поиск точки по описанию адреса. Запросы такого типа кэшируются в таблице **address_cache_forward**. Нагрузка на таблицу составит: `93 * 0.75 * 0.5 = 35 QPS`

Нагрузка на **address_cache_reverse** - 25% запросов, то есть: `93 * 0.25 * 0.5 = 12 QPS`

Таблица **node** обслуживает запросы на построение маршрутов. При построении выполняется 1 запрос, затрагивающий таблицу **node**. При этом 20% запросов будет кэшироваться в **routes_cache**, то есть число запросов к таблицам составит 0.8%. Тогда: `37 * 0.8 * 0.5 = 15 QPS`

Таблица **routes_cache** кэширует запросы на построение маршрутов. Нагрузка на таблицу составит: `37 * 0.5 = 19 QPS`

| Таблица               | QPS   |
|-----------------------|-------|
| address               | 38    |
| address_cache_forward | 35    |
| address_cache_reverse | 12     |
| nodes                 | 15   |
| relations             | 19   |
| relation_members      | 19   |
| routes_cache          | 19    |
...
* **Кластер Pelias**. Поиск - это CPU intensive операция, поэтому **8 CPU 32 GB RAM** Pelias обеспечивает 250 RPS. Таким образом, потребуется 38 QPS / 250 < 1 сервер с репликами, плюс 1 резервный. Для хранения данных используется **1 TB SSD**.
